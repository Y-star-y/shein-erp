...
export function MappingForm({
  ...
  const selectedCompanySku = companySkus.find((item) => item.id === value.internalProductId);



  const extraOptions = useMemo<SelectOption[]>(() => {
    ...
    return [

      {
        ...
        label: `${selectedCompanySku.id} / ${getProductDisplayName(selectedCompanySku)}（已停用）`,

        value: selectedCompanySku.id,
      ...
      <div className="modal-actions">
        ...
        <span>
          ...
          {mode === "create"
            ...
            ? "平台 SKU 为全局唯一匹配键；多个店铺的平台 SKU 可指向同一内部商品。"
