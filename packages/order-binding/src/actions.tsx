...
import { useErpStore, type OrderBindRequest, type OrderBindResult, type PlatformSkuMapping, type UnmappedSkcGroup } from "@shein-erp/shared";
...
export function useOrderBindingActions(onBound?: () => void) {
  const { setMappings, setModal, modal, pushToast } = useErpStore();
  ...
  const saveBind = useCallback(
    (event: React.FormEvent<HTMLFormElement>) => {
      ...
      const payload: OrderBindRequest = {
        ...
        internalProductId: modal.value.internalProductId.trim(),
        ...
        spec: modal.value.spec.trim(),
        articleNo: modal.value.articleNo.trim(),
      ...
      requestJson<OrderBindResult>("/api/orders/bind", {
        ...
        .then((result) => {
          ...
          pushToast("success", `已绑定平台 SKU，更新了 ${result.updatedLineCount} 条订单行`);
          setModal(null);
          onBound?.();
        })
        .catch((error: Error) => pushToast("error", error.message));
    },
    [modal, onBound, pushToast, setMappings, setModal],
  );
