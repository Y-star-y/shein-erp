...
import { EmptyBlock, includesQuery, PageHeader, type OrderImportResult } from "@shein-erp/shared";
...
export function ImportTab({ onImported }: { onImported: () => void }) {
  ...
  async function handleUpload(file: File) {
    ...
    try {
      ...
      const body = (await response.json()) as OrderImportResult & { error?: string };
      if (!response.ok) {
      ...
      <section className="table-panel">
        <div className="table-toolbar" style={{ flexDirection: "column", alignItems: "stretch", gap: 16 }}>
          <Input
            ...
            status={error && !storeName.trim() ? "error" : undefined}
            value={storeName}
          ...
          {result ? (
            <Alert
              showIcon
              type="info"
              message={`导入完成：共 ${result.total} 行，已映射 ${result.mapped} 行，待绑定 ${result.unmapped} 行`}
